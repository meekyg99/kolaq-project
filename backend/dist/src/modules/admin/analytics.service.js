...
let AnalyticsService = class AnalyticsService {
    ...
    async getProductPerformance(query) {
        ...
        const productsWithDetails = await Promise.all(productSales.map(async (item) => {
        ...
        const inventory = products.map((product) => {
            ...
            return {
                ...
                variants: product.variants.map((v) => ({
                    ...
                    stock: v.stock,
                    status: v.stock <= outOfStockThreshold
                        ? 'OUT_OF_STOCK'
                        : v.stock <= lowStockThreshold
                            ? 'LOW_STOCK'
                            : 'IN_STOCK',
                })),
                status: totalStock <= outOfStockThreshold
                    ? 'OUT_OF_STOCK'
    ...
    async getInventoryForecast(days = 30, productId) {
        ...
        const productGroups = historicalSales.reduce((acc, item) => {
        ...
        for (const [prodId, sales] of Object.entries(productGroups)) {
            ...
            const projectedStockout = dailyAverage > 0 ? Math.floor(currentStock / dailyAverage) : null;
    ...
    async getCustomerMetrics(query) {
        ...
        const totalCustomerValue = orders.reduce((sum, o) => sum + Number(o.total), 0);
        const averageCustomerValue = uniqueCustomers > 0 ? totalCustomerValue / uniqueCustomers : 0;
        return {
            uniqueCustomers,
            returningCustomers,
    ...
    getDateRange(query) {
        ...
        let startDate;
        if (query.range === analytics_dto_1.TimeRange.CUSTOM && query.startDate) {
            startDate = new Date(query.startDate);
        }
        else {
            startDate = new Date();
            switch (query.range) {
                case analytics_dto_1.TimeRange.TODAY:
