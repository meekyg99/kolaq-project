...
export const LAGOS_ZONES = {
  Island: ['Eti Osa', 'Ikoyi', 'Lagos Island', 'Victoria Island', 'Lekki', 'Ajah', 'Ibeju-Lekki'],
  Mainland: ['Agege', 'Ajeromi-Ifelodun', 'Alimosho', 'Amuwo-Odofin', 'Apapa', 'Badagry', 'Epe', 'Ifako-Ijaiye', 'Ikeja', 'Ikorodu', 'Kosofe', 'Lagos Mainland', 'Mushin', 'Ojo', 'Oshodi-Isolo', 'Shomolu', 'Surulere']
};
...
export function getShippingZone(state: string, lga?: string): string {
  const stateUpper = state.toUpperCase();
  
  if (stateUpper === 'LAGOS' && lga) {
    const lgaZone = getLagosZone(lga);
    return lgaZone === 'Island' ? 'Lagos-Island' : 'Lagos-Mainland';
  }
  
  if (stateUpper === 'LAGOS') return 'Lagos-Mainland';
  if (['OGUN', 'OYO', 'OSUN', 'ONDO', 'EKITI'].includes(stateUpper)) return 'South-West';
  if (['ANAMBRA', 'ENUGU', 'IMO', 'ABIA', 'EBONYI'].includes(stateUpper)) return 'South-East';
  if (['RIVERS', 'BAYELSA', 'DELTA', 'AKWA IBOM', 'CROSS RIVER', 'EDO'].includes(stateUpper)) return 'South-South';
  if (['FCT', 'NIGER', 'KOGI', 'KWARA', 'NASARAWA', 'PLATEAU', 'BENUE'].includes(stateUpper)) return 'North-Central';
  if (['KADUNA', 'KANO', 'KATSINA', 'KEBBI', 'SOKOTO', 'ZAMFARA', 'JIGAWA'].includes(stateUpper)) return 'North-West';
  if (['BORNO', 'ADAMAWA', 'YOBE', 'TARABA', 'GOMBE', 'BAUCHI'].includes(stateUpper)) return 'North-East';
  
  return 'North-Central';
}

export function getLagosZone(lga: string): 'Island' | 'Mainland' {
  const lgaUpper = lga.toUpperCase();
  const islandLGAs = LAGOS_ZONES.Island.map(l => l.toUpperCase());
  return islandLGAs.includes(lgaUpper) ? 'Island' : 'Mainland';
}

export function getShippingRate(state: string, lga: string | undefined, currency: 'NGN' | 'USD'): number {
  const zone = getShippingZone(state, lga);
  return SHIPPING_ZONES[zone as keyof typeof SHIPPING_ZONES][currency];
}
